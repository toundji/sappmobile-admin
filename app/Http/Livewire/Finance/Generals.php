...
class Generals extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->type == "day") {
            $this->filter = "Jour : ".$this->date;
            $transports = Transport::where('status', "DEMANDE")->whereDate('created_at', $this->date)->get();
        } elseif ($this->type == "month") {
            $this->filter = "Mois : ".$this->month;
            $month = explode("-", $this->month)[1];
            $year = explode("-", $this->month)[0];
            $transports = Transport::where('status', "DEMANDE")->whereYear('created_at', $year)->whereMonth('created_at', $month)->get();
        } elseif ($this->type == "year") {
            $this->filter = "Année : ".$this->year;
            $transports = Transport::where('status', "DEMANDE")->whereYear('created_at', $this->year)->get();
        } elseif ($this->type == "interval") {
            $this->filter = "Intervalle : [".$this->start.", ".$this->end."]";
            $transports = Transport::where('status', "DEMANDE")->whereBetween('created_at', [$this->start, $this->end])->get();
        }
        if ($this->payement_mode != "all") {
            $transports = $transports->where('payement_mode', $this->payement_mode);
        }
        $this->total = $transports->sum('price');
        return view('livewire.finance.generals', [
